...
# terraform destroy fails due to bug https://github.com/vmware/terraform-provider-avi/issues/376
...
resource "avi_serviceenginegroup" "avi_se_group" {
  ...
  vcenter_datastores {
    managed_object_id = data.vsphere_datastore.datastore.id
  }
  vcenter_clusters {
    cluster_refs = [ "https://${var.avi_controller_ip}/api/vimgrclusterruntime/${data.vsphere_compute_cluster.cluster.id}-${data.avi_cloud.avi_cloud.uuid}" ]
    include = true
  }
  # vcenter_hosts {
  #   host_refs = [ "https://${var.avi_controller_ip}/api/vimgrhostruntime/${data.vsphere_host.host.id}-${data.avi_cloud.avi_cloud.uuid}" ]
  #   include = true
  # }
}
